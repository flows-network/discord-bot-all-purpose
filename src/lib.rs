    ...
    async fn handle(&self, msg: Message, help_msg: String, placeholder_text: String) {
        ...
        match content.as_str() {
            ...
            "/summarize" => {
                ...
                log::info!("Started Chinese translation for {}", channel_id);
            }
            "/code" => {
            ...
            text => {
                ...
                if text.is_empty() {
                    ...
                    for url in urls {
                        ...
                        self.chat(&detected, &co, channel_id, placeholder.id).await;
                    }
                } else {
                    log::info!("received text: {}", text);
                    self.chat(text, &co, channel_id, placeholder.id).await;
                }
            }
        }
    }
...
impl App {
    async fn chat(
        ...
        match self.openai.chat_completion(&channel_id.to_string(), text, co).await {
            Ok(r) => {
